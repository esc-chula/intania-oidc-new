...
export const studentRouter = createTRPCRouter({
    ...
    update: protectedProcedure.input(z.object({
        ...
        return await ctx.db.update(students).set({ ...input, updatedAt: new Date(), id: undefined }).where(eq(students.id, id));
    }),
    getMiscInfo: publicProcedure
        .query(async ({ ctx }) => {
            const countries = await ctx.db.query.countries.findMany();
            const religions = await ctx.db.query.religions.findMany();
            const engineeringDepartments = await ctx.db.query.engineeringDepartments.findMany();
            const familyMemberStatuses = await ctx.db.query.familyMemberStatuses.findMany();
